...
const ProfitableProducts = ({ discountProduct, error }) => {
  ...
  const getDiscountProduct = (perPage = 12, page = 1) => {
    setLoader(true);
    ProductApi.getDiscount({ perPage, page, user_address_id })
      .then((response) => {
...
export async function getServerSideProps(ctx) {
  ...
  return axiosService
    .get(`/api/v1/rest/products/discount`, { params })
    .then((res) => ({ props: { discountProduct: res.data, error: null } }))
    .catch((error) => ({
      props: { discountProduct: {}, error: error.response.data },
    }));
}
